...
figs("img","Estimated Model Error Rate")
capimg = figs("img")

figs("box1","First 4 stratifying features")
capbox1 = figs("box1")

figs("box2","pS6 Median Features")
capbox2 = figs("box2")
...
```
##Model Construction:
When the two sample groups are inspected to identify cluster features that differ among them, each of the sample involved is assigned 
to one of the two or more sample groups (e.g. stimulated and unstimulated). Following this, regularized classification models are constructed. This is done using the calculated cluster features that act as the regressors/independent variables of the sample groups. However, it is important to realize that not all of the calculate cluster features will be useful in differentiating sample groups. Therefore, only a subset of the features is used, and the models are generated using the nearest shrunken centroid (pamr) and lasso-regularized logistic regression (glmnet) methods. Both of these methods build a series of predictive models by using the selected subsets of supplied regressors. To limit the number of the features in any given model, a regularization threshold is used. Since, we do not know which subset of cluster feature can best stratify the sample group, a set of models are built using a range of regularization thresholds. To select one from this set of models, cross validation is performed on all the models and the best is selected that meets the constraints specified by the user. 
When it is required to identify cluster features that are predictive of the survival times of the sample, these are specified by the user. This is followed by the construction of many lasso-regularized cox proportional-hazards models, wherein provided cluster features act as regressors of the sample survival time. Here as well, the number of features included in a model is restricted by regularization threshold. As it is unknown, which subset acts as a best predictor, a set of models are built using a range of regularization threshold, which are evaluated using K-fold cross validation. The regularization threshold that produces the model with the best total goodness of fit is used to constrain the final model.

##Calculation of Regularization Thresholds
This function generates a range of regularization thresholds for further model construction. It is in a vector form.
...
The quality of results can be assessed using model error rate plots. If a constructed model has a lower estimated error rate, this means that the model under investigation has identified some subset of cluster features which are robust predictors of a sample’s class. These identified features in turn depict a behavior that is unique to that class, and are hence, stratifying subsets of interests. On the other hand, a model with a high error rate has features that do not differ between sample classes and hence are not useful stratifying features. Thus, interpretation should be done only from models having an acceptable error rate. 
When such models with accepted error rates have been constructed, a regularization threshold must be chosen that will be used to constrain the final model constructed from all the sample features. The aim is to obtain the smallest but most informative subset of features that differ between classes. This is done by selecting a threshold that results in a simplest model with an acceptable error rate. The same is true when seeking to identify many or all the features that differ between sample groups. The difference being, the model will be of complex nature and would include a regularization constraint. 
...
Nonzero features of the final model, which is determined by the regularization thresholds, are the set of features that best differentiate sample groups. The values of these relevant features for each sample are shown in box plots. Equally important as the identified stratifying features are the phenotypes of corresponding clusters. To determine these phenotypes, densities plots or scatter plots of lineage markers in cluster cells are drawn, along with the plots of same markers in other cells. This allows for a comparative analysis as to how much each marker is enriched in cluster cells. Related stratifying subsets that have similar behavior may be identified by highlighting relevant subsets in plots of the clustering hierarchy.

##Constructing Figures
```{r fig.align='center',fig.cap=capimg}
...
#arrows(78, thresholdCVRates$fdr[78], 78, thresholdCVRates$fdr[78] + sd(thresholdCVRates$fdr), length = 0.1, col=3, angle=90)
#arrows(85, thresholdCVRates$fdr[85], 85, thresholdCVRates$fdr[85] + sd(thresholdCVRates$fdr), length = 0.1, col=3, angle=90)
#arrows(92, thresholdCVRates$fdr[92], 92, thresholdCVRates$fdr[92] + sd(thresholdCVRates$fdr), length = 0.1, col=3, angle=90)
#arrows(99, thresholdCVRates$fdr[99], 99, thresholdCVRates$fdr[99] + sd(thresholdCVRates$fdr), length = 0.1, col=3, angle=90)


par(new=TRUE)

axis(1, at=c(0,20,40,60,80,100),labels=c(10,8,6,4,2,0))
...
```
##Results
The descriptive cluster features were used to train a nearest shrunken centroid classifier (pamr) of the sample's simulation group (reference or BCR-treated) at a range of regularization thresholds. Cross-validation and permutation tests were used to estimate and plot classification error rates and feature false discovery rates of each model respectively. 
In order to determine all features that differed between the different sample groups, error plot produced above was used to identify the smallest regularization threshold that produced a model with acceptable cross validation accuracy and an estimated feature FDR of < 1%. Using this, Citrus constrained the final classificaiton model from all sample. From the original set of 490 cluster features, a subset of 159 was chosen as which differed between the two simulation conditions.

##Fold Features
```{r}
citrus.clusteringfolds = citrus.clusterAndMapFolds(citrus.combinedFCSSet,
                                                   clusteringColumns,
                                                   labels, 
                                                   nFolds = 1)

citrus.foldFeatureSet = citrus.calculateFoldFeatureSet(citrus.clusteringfolds,citrus.combinedFCSSet)

citrus.regressionResult = citrus.endpointRegress(modelType="pamr",
                                                 citrus.foldFeatureSet,labels,
                                                 family="classification")

plot(citrus.regressionResult,
     outputDirectory = "F:/Academics/Masters/UZH/material-master/Project/08Jan/sta426prj-master/fold_citrus_8jan_op",
     citrus.clusteringfolds,
     citrus.foldFeatureSet,
     citrus.combinedFCSSet)
```
##Data Transformation for Box Plot
```{r}
#DataTransformation
...
for(i in 1:490){
  ...
  for(j in 1:16){
    c <- 1 - m
    if(medianFeatures[j,i] < 0){
      medianFeatures_4[j,i] <- log(medianFeatures_4[j,i] + c)
      
    }
    else
      medianFeatures_4[j,i] <- medianFeatures_4[j,i]
  }
}


for(i in 1:490) {
  maxi <- max(medianFeatures_4[,i])
  for(j in 1:16){
    medianFeatures_4[j,i] <- (medianFeatures_4[j,i]/maxi)*4
    }
}
```
#BoxPlot for first 4 features

The box plots below show the transformed data scaled between 0 and 4 for all the features.
```{r fig.align='center',fig.cap=capbox1}
#Box plot for the first 4 features.
list_fea <- modelfeatures$cv.min$features[1:159]
col_med <- colnames(medianFeatures_4)

c = vector(mode="integer")
k <- 1
for(i in 1:length(list_fea)){
  for(j in 1:length(col_med)){
    if(list_fea[i] == col_med[j]){
      c[k]<-j
      k<-k+1
    }
  }
...
for(j in 1:4){
  boxplot(medianFeatures_4[1:8,c[j]],medianFeatures_4[9:16,c[j]],
          horizontal = TRUE, names=c('BCR-XL','Ref'), ylim = c(0,4))
}
```

##BoxPlot for pS6 features
```{r fig.align='center',fig.cap=capbox2} 
#PS6
value <- vector(mode ="numeric", length=0)
value[grep("pS6",list_fea)] <- 1
value

d = vector(mode="integer")
k <- 1
for(i in 1:length(pS6)){
  for(j in 1:length(col_med)){
    if(pS6[i] == col_med[j]){
      d[k]<-j
      k<-k+1
...
for(j in 1:length(d)){
  boxplot(medianFeatures_4[1:8,d[j]],medianFeatures_4[9:16,d[j]],
          horizontal = TRUE, names=c('BCR-XL','Ref'), ylim=c(0.0, 4.0))
}
```
